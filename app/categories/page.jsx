...
function CategoryCard({ category }) {
    return <Link href={`/categories/${category?.id}`}>
        <div className="flex flex-col items-center justify-center gap-2 rounded-xl p-6 bg-slate-400 hover:bg-slate-500 hover:scale-105 transition-all">
            <img className="h-28 w-28 object-cover rounded-full" src={category?.iconURL} alt="" />
            <h1 className="font-bold text-white text-2xl">{category?.name}</h1>
        </div>
    </Link>
}